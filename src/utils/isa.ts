 ...
 */
export const isMetricCol = (col: Column | null | undefined) =>
  !isDimensionCol(col) && isNumeric(col);
export const findDefaultDimensionName = (cols: Column[]) =>
  cols.find(isDimensionCol)?.name;
export const findDefaultMetricName = (cols: Column[]) =>
  cols.find(isMetricCol)?.name;
